...
public class RobotEnviroment {
	...
	public static List<TrackedBeacon> extractBeacons( Map<Color, List<TrackedColor>> detectedObjects ) {
		...
		for ( Beacon b : Beacon.values() ) {
			...
			if ( upperColor == null || upperColor.size() == 0 || lowerColor == null || lowerColor.size() == 0 ) continue;
	...
	public static Point calcPosition( List<TrackedBeacon> beacons, Mat homography ) {
		...
		if ( beacons == null || beacons.size() < 2 || homography == null ) return null;
		...
		if ( d0 < 0 || d1 < 0 || p0 == null || p1 == null ) return null;
		...
		if ( p0.x == MIN_X && p1.x == MIN_X ) {
			if ( (p0.y == MIN_Y && p1.y == HALFWAY_Y) || (p0.y == MIN_Y && p1.y == MAX_Y) ) // beacons(
																							// 0:0,0:75
																							// )
																							// or
																							// beacons(
			...
			else if ( p0.y == HALFWAY_Y && p1.y == MAX_Y ) // left beacon is
															// 0:75, right one
															// is 0:150
				robot = new Point( lc, HALFWAY_Y + la );

		} else if ( p0.x == MAX_X && p1.x == MAX_X ) {
			if ( p0.y == HALFWAY_Y && p1.y == MIN_Y ) // left beacon is 150:75,
														// right one is 150:0
				robot = new Point( MAX_X - lc, HALFWAY_Y - la );
			else if ( (p0.y == MAX_Y && p1.y == HALFWAY_Y) || (p0.y == MAX_Y && p1.y == MIN_Y) ) // beacons(
																									// 150:150,150:75
																									// )
																									// or
																									// beacons(
			...
			if ( (p0.x == MAX_X && p1.x == HALFWAY_X) || (p0.x == MAX_X && p1.x == MIN_X) ) // beacons(
																							// 150:0,75:0
																							// )
																							// or
																							// beacons(
			...
			else if ( p0.x == HALFWAY_X && p1.x == MIN_X ) // left beacon is
															// 75:0, right one
															// is 0:0
				robot = new Point( HALFWAY_X - la, lc );

		} else if ( p0.y == MAX_Y && p1.y == MAX_Y ) {
			if ( (p0.x == MIN_X && p1.x == HALFWAY_X) || (p0.x == MIN_X && p1.x == MAX_X) ) // beacons(
																							// 0:150,75:150
																							// )
																							// or
																							// beacons(
			...
			else if ( p0.x == HALFWAY_X && p1.x == MAX_X ) robot = new Point( HALFWAY_X + la, MAX_Y - lc ); // left
																											// beacon
																											// is
																											// 75:150,
																											// right
	...
	public static TrackedBall findBall( Map<Color, List<TrackedColor>> detectedObjects ) {
		...
		if ( ballColors == null || ballColors.size() < 1 ) return null;
		...
		for ( TrackedColor c : ballColors ) {
			...
			if ( c.getBorders().height > maxHeight && c.getBorders().height > MIN_BALL_DIAMETER ) {
				maxHeight = c.getBorders().height;
				largestColor = index;
			}

			if ( c.getBorders().width > maxWidth && c.getBorders().width > MIN_BALL_DIAMETER ) {
				maxHeight = c.getBorders().width;
				largestColor = index;
			}
		...
		if(largestColor >= 0) {
			...
			ball.setBallColor( ballColors.get( largestColor ) );
		}
	...
	public static Point parseCoords( String s ) {
		if ( !hasCoordFormat( s ) ) return null;

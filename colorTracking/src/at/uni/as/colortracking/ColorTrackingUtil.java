...
public class ColorTrackingUtil {
	...
	public static ArrayList<Mat> convertRGB2RG(Mat image) {
		...
		Mat b= new MatOfFloat();
	      	...
	      	Core.divide(chR, new Scalar(255.0),r,CvType.CV_64F);// dividing it at first in order not to lose data
	      	Core.divide(chG, new Scalar(255.0), g,CvType.CV_64F);// and to have scaled data to values 0-255
	...
	public static List<MatOfPoint> getContours(Mat img) {
		...
		Imgproc.findContours(clone, contours, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE);
	...
	public static Mat getForegroundImage(int x, int y, Mat image) {
		int cols = image.cols();
	        int rows = image.rows();
	         ...
	         Mat cli = new Mat();
	         Iterator<MatOfPoint> each1 = contours.iterator();
